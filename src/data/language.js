export const language = {
  en: {
    topSection: {
      header: "Invest in the Lucrative Dubai Real Estate Market Today!",
      paragraph:
        "Scorpion Property provides a wide range of investment opportunities including high-profile real estate properties, offering the highest ROI.",
      paragraph1:
        "Contact Us to Know About Incredible Properties for Sale with Attractive Offers.",
    },
    whyChooseUs: {
      header: "About Scorpion Property",
      firstLine: "Your Real Estate Partner in Dubai",
      paragraph1:
        "Scorpion property is a real estate company, located at the heart of Dubai “JBR Walk”. We deal in buying, selling, renting, and managing properties all over Dubai. The company strives to offer a new level of service and to provide the best value possible to our clients through a dedicated & experienced team of real estate agents. We adopt a tailored approach for every client to ensure we can gain a detailed understanding, put your best interests first and exceed expectations.",
    },
    events: {
      header: "Dubai Property Expo by Damac",
      firstLine: "Sep 2022, Ahmedabad",
      paragraph1:
        "Come and meet the real estate experts & get to know about the most luxurious properties on offer in Dubai by Damac. Connect with key stakeholders in Dubai real estate and find unparalleled investment opportunities, offering the best ROI.",
    },
    footer: {
      address: "Head Office",
      PrivacyTitle: "Privacy Policy",
      paragraph: `We have implemented measures designed to secure your personal
        information from accidental loss and from unauthorized access,
        use, alteration and disclosure. All information you provide to
        us is stored on our secure servers behind firewalls. Any
        payment transactions will be encrypted using SSL technology.`,
    },
  },
  gu: {
    topSection: {
      header: "આજે દુબઈ ના આકર્ષક રીયલ એસ્ટેટ માર્કેટમાં રોકાણ કરો!",
      paragraph:
        "સ્કોર્પિયન પ્રોપર્ટી ઉચ્ચતમ વળતર ઓફર કરતી હાઇ-પ્રોફાઇલ રિયલ એસ્ટેટ પ્રોપર્ટીઝ સહિત રોકાણની વિશાળ શ્રેણી પૂરી પાડે છે.",
      paragraph1:
        "આકર્ષક ઑફર્સ સાથે વેચાણ માટે અતુલ્ય પ્રોપર્ટીઝ વિશે જાણવા માટે અમારો સંપર્ક કરો .",
    },
    whyChooseUs: {
      header: "વિશે વધુ માહિતી",
      firstLine: "દુબઈમાં તમારો રિયલ એસ્ટેટ પાર્ટનર",
      paragraph1:
        "Scorpion property એ એક રિયલ એસ્ટેટ કંપની છે, જે દુબઈ 'JBR Walk' ના કેન્દ્રમાં સ્થિત છે. અમે સમગ્ર દુબઈમાં મિલકતો ખરીદવા, વેચવા, ભાડે આપવા અને મેનેજ કરવાનું કાર્ય કરીએ છીએ. કંપની રિયલ એસ્ટેટ એજન્ટો સમર્પિત અને અનુભવી ટીમ દ્વારા નવા સ્તરની સેવા પ્રદાન કરવા અને અમારા ગ્રાહકોને શક્ય તેટલું શ્રેષ્ઠ મૂલ્ય પ્રદાન કરવાનો પ્રયાસ કરે છે. અમે દરેક ક્લાયન્ટ માટે એક સર્વોતમ અભિગમ અપનાવીએ છીએ તેની ખાતરી કરવા માટે કે અમે વિગતવાર સમજ મેળવી શકીએ, તમારા શ્રેષ્ઠ હિતોને પ્રથમ સ્થાન આપી શકીએ અને તમારી અપેક્ષાઓથી વધી શકીએ.",
    },
    events: {
      header: "દુબઈ પ્રોપટી એક્ષ્પો Damac દ્વારા",
      firstLine: "સપ્ટે 2022, અમદાવાદ",
      paragraph1:
        "પધારો અને રિયલ એસ્ટેટ નિષ્ણાતોને મળો અને Damac દ્વારા દુબઈમાં ઓફર કરવામાં આવતી સૌથી વૈભવી મિલકતો વિશે જાણો. દુબઈ રિયલ એસ્ટેટના મુખ્ય હિસ્સેદારો સાથે જોડાઓ અને શ્રેષ્ઠ વળતર ઓફર કરીને અદભુત રોકાણની તકો શોધો.",
    },
    footer: {
      address: "મુખ્ય કાર્યાલય",
      PrivacyTitle: "ગોપનીયતા નીતિ",
      paragraph:
        "અમે તમારી વ્યક્તિગત માહિતીને આકસ્મિક નુકસાન અને અનાધિકૃત ઍક્સેસ, ઉપયોગ, ફેરફાર અને જાહેરાતથી સુરક્ષિત કરવા માટે રચાયેલ પગલાં અમલમાં મૂક્યા છે. તમે અમને પ્રદાન કરો છો તે તમામ માહિતી ફાયરવોલ પાછળ અમારા સુરક્ષિત સર્વર્સ પર સંગ્રહિત છે. કોઈપણ ચુકવણીના વ્યવહારો SSL ટેકનીકનો ઉપયોગ કરીને એન્ક્રિપ્ટેડ કરવામાં આવશે.",
    },
  },
};
